...
以下のいずれかに該当するとき、**作業完了と同時に**ADRを起票する。

- ライブラリ・ツールの選定（例: sqlc vs GORM）
- アーキテクチャ上の構成決定（例: docker-compose vs Terraform）
- 優先順位・フェーズ計画の決定
- テスト戦略の決定
- センシティブ／非センシティブの判定を行った場合

ADRのファイル名は `NNNN-kebab-case-title.md` とし、番号は連番とする。
